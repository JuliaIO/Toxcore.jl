...
function MyFriendRequest(tox::Ptr{Tox}, public_key::Ptr{Uint8}, message::Ptr{Uint8}, length::Csize_t, user_data::Ptr{Void})
    ...
    tox_friend_add_norequest(my_tox, public_key, C_NULL)
    nothing
end
...
function main()
    ...
    tox_self_get_address(my_tox, my_address)
    ...
    res = tox_bootstrap(my_tox, BOOTSTRAP_ADDRESS, BOOTSTRAP_PORT, bootstrap_pub_key, C_NULL)
    if !Bool(res)
        println("shiiiiiiiit")
    end 
    while true
