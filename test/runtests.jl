...
function main()
    ...
    my_tox = Toxcore.CInterface.tox_new(C_NULL, C_NULL, 0, C_NULL)

    #Register the callbacks */
    Toxcore.CInterface.tox_callback_friend_request(my_tox, MyFriendRequestCallback, C_NULL)
    Toxcore.CInterface.tox_callback_friend_message(my_tox, MyFriendMessageCallback, C_NULL)
    my_address = zeros(Uint8,TOX_ADDRESS_SIZE)
    Toxcore.CInterface.tox_self_get_address(my_tox, my_address)
    println(uppercase(bytes2hex(my_address)))
    #Define or load some user details for the sake of it */
    Toxcore.CInterface.tox_self_set_name(my_tox, MY_NAME, length(MY_NAME), C_NULL) # Sets the username
    Toxcore.CInterface.tox_self_set_status_message(my_tox, MY_STATUS_MESSAGE, length(MY_STATUS_MESSAGE), C_NULL) # Sets the status message

    # Set the user status TOX_USER_STATUS_NONE. Other possible values:
    # TOX_USER_STATUS_AWAY TOX_USER_STATUS_BUSY 
    #tox_self_set_status(my_tox, 0)

    #Bootstrap from the node defined above */
    res = Toxcore.CInterface.tox_bootstrap(my_tox, BOOTSTRAP_ADDRESS, BOOTSTRAP_PORT, bootstrap_pub_key, C_NULL)
    if !Bool(res)
        println("shiiiiiiiit")
    end 
    while true
        Toxcore.CInterface.tox_iterate(my_tox); # will call the callback functions defined and registered
        sleep(Toxcore.CInterface.tox_iteration_interval(my_tox)/1000) # sleep until the next iteration should happen
    end

    Toxcore.CInterface.tox_kill(my_tox)
    println("success")
end
